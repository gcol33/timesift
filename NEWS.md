...
* The penalised core's binomial fit is at glmnet's speed on one thread, at the shipped
  `thresh = 1e-8`, one notch tighter than glmnet's default. The path reads the columns once per penalty rather than twice,
  a binomial fit ends each penalty on the one reweighting the next penalty opens with, and a
  reweighted least squares step is judged on its own move, which is glmnet's test (#77). A
  Gaussian fit folds the root of each case weight into the design, so a coordinate's step reads
  two streams where it read three.
...
* Timings, one cross-validated path, installed builds, against `cv.glmnet` at its default on the
  ...
  glmnet's own threshold of `1e-7`), and 0.12 s
  against 0.25 s for the four-response learner on a small ill-conditioned design (60 units, 18
  columns). Binomial fits on a well-conditioned design move in the fourth decimal of a
  coefficient at most, and choose the same penalty.
